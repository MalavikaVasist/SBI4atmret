...
from .Loss import BNPELoss, NPELoss
from ...scripts.plotting import plot_results



def train_epoch(estimator, step, trainsets, simulator, pipe, config: Config):
    estimator.train()
    start = time.time()
...
def validate_epoch(estimator, validsets, simulator, pipe, step, config: Config):
    estimator.eval()
    loss_valid_list = []
    with torch.no_grad():
...
def run_training(config: Config, i: int, datasets, simulator, observation, checkpoint_fn=None, checkpoint_interval=50):
    ...
    for epoch in tqdm(range(model_configs["epoch_fin"][i], model_configs["epochs"][i]), unit='epoch'):
