...
class MiriGeminiHSTcloudyPipe(BasePipe):
    def __init__(self, config):
        super().__init__(config)
        

    def modify_spec(self, batch_dict):
        cf = batch_dict["cloudy"]
        theta, x   = cf["miri"]
        thetag, xg = cf["gemini"]
    ...
    def modify_prior(self, batch_dict):
        ...
        idxg = self.domain.param_index["cloudy_gemini"]["bfactor_noise_g"]
        idxh = self.domain.param_index["cloudy_hst"]["bfactor_noise_h"]
    ...
    def build_input(self, batch_dict):
        ...
        ## miri
        idx_scalingm = self.domain.parameter_idx["cloudy_miri"]["bfactor_noise_miri"]
        b = torch.unsqueeze(thetam[:, idx_scalingm], 1)

        ## gemini
        idx_c = self.domain.parameter_idx["cloudy_gemini"]["mxture_fraction"]
        idx_scalingg =  self.domain.parameter_idx["cloudy_gemini"]["Cushing_scale_factor_g"]
        thetag = np.delete(theta, [idx_c, idx_scalingg], axis=1)

        ## hst
        idx_scalingh =  self.domain.parameter_idx["cloudy_hst"]["bfactor_noise_gemini"] ##inverted
